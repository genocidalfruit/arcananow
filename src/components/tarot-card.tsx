...
export function TarotCard({ card, isFlipped, onFlip, label, className }: TarotCardProps) {
  ...
  return (
    <div className={cn("flex flex-col items-center space-y-2", className)}>
      ...
      <div
        className={cn(
          `${cardWidth} ${cardHeight} rounded-lg shadow-xl cursor-pointer`,
          "group"
        )}
        onClick={onFlip}
      ...
      >
        <div
          className={cn(
            `relative w-full h-full rounded-lg transition-transform duration-700 ease-in-out [transform-style:preserve-3d]`, // Apply preserve-3d
            isFlipped ? 'rotate-y-180' : '' // Conditional rotation
          )}
        >
          {/* Card Back */}
          <div className="absolute w-full h-full rounded-lg [backface-visibility:hidden] bg-primary border-2 border-accent flex items-center justify-center"> {/* Apply backface-hidden */}
          ...
          {/* Card Front */}
          <div className="absolute w-full h-full rounded-lg [backface-visibility:hidden] rotate-y-180 bg-card overflow-hidden"> {/* Apply backface-hidden and initial rotation for front */}
            {card ? (
